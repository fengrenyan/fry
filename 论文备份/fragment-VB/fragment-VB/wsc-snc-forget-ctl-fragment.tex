...
\caption{An initial $V$-partition algorithm}% 算法名字
%\LinesNumbered %要求显示行号
\KwIn{finite transition system $\Hm=(S, R, L)$ on $\Ha=\{q_1, ..., q_n\}$ and $V =\{p_1, ..., p_m\} \subseteq \Ha$}% 输入参数
\KwOut{Initial $V$-partition $\Pi_V$}% 输出
...
\caption{An V-partition refinement algorithm}% 算法名字
%\LinesNumbered %要求显示行号
\KwIn{finite transition system $\Hm=(S, R, L)$ on $\Ha$, $s\in S$ and $V\subseteq \Ha$}% 输入参数
\KwOut{$\overline V$-quotient space $S/\Hb$}% 输出
$\Pi'=\{S\}$\;
$\Pi=\Pi_{V}$ \;
\Repeat
{$\Pi = \Pi'$}{
