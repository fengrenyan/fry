...
\label{chapter05:sec:model}
	...
	为此，本节证明$\CTL$公式的遗忘结果在有界的情形下可以表示成其模型的特征公式的析取，并提出一种基于模型的方法计算遗忘。%，且探索了如何使用遗忘计算最弱充分条件和知识更新。
...
由此可以得出基于模型的遗忘算法（算法\ref{alg:compute:forgetting:by:VB}），其主要思想是：枚举所有与原公式的模型具有$V$-互模拟关系的初始结构，这些初始结构的特征公式的吸取记为从原公式中遗忘掉$V$中的元素后得到的结果。算法\ref{alg:compute:forgetting:by:VB}的正确性可由引理\ref{lem:models:formula}和定理\ref{CF}保证。
