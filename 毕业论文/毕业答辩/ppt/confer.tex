...
\begin{document}
			...
			\only<2>{
				\begin{itemize}
					...
					\begin{itemize}
						...
						\item 能很好的表达系统要求的各种属性：
						\begin{itemize}
							\item \textcolor{orange!80}{安全属性（Safety properties）}：something
							bad never happens. \textcolor{blue!70}{($\ALL \GLOBAL \neg \varphi$)}
							\item \textcolor{orange!80}{活性属性（Liveness	properties）}：something good will eventually happen. \textcolor{blue!70}{($\ALL \GLOBAL \ALL \FUTURE \varphi$)}
							\item \textcolor{orange!80}{持续属性（Persistence properties）}：eventually for ever a certain proposition holds. \textcolor{blue!70}{($\ALL \FUTURE \ALL \GLOBAL \varphi$)}
							\item \textcolor{orange!80}{公平属性（Fairness properties）}：does,
							under certain conditions, an event occur repeatedly? \textcolor{blue!70}{（一种约束：在约束$fair = \GLOBAL \FUTURE a$下$\ALL \GLOBAL(a\rto \ALL \FUTURE b)$是否成立？）}
						\end{itemize} 
					\end{itemize}
	...
	%			\end{itemize}
	%			\item $CTL$和$\mu$-演算的遗忘在推理问题上的复杂性是什么？
	%		\end{itemize}
	%	\end{block}
	%}
	%\end{frame}
	...
	\begin{frame} 
		...
		{\footnotesize 
			...
			%		\begin{itemize}
			%			\item {\em 模型}：满足公式$\varphi$的初始$\Ind$-结构称为$\varphi$的一个模型；
		...
		\begin{figure}
			...
			\begin{tikzpicture}[scale=0.8]
				...
				\node at(0,4) {{\scriptsize $
						...
						\end{array}
						\right.$}};
				\draw[dashed, color=orange!40] (-3.8,4.5) rectangle (3.5,3.5);
				...
				\node[circle,minimum width =30pt, minimum height =30pt, fill=red!20] at(-3,1) {{\scriptsize 可靠且完备\linebreak 的算法}};
				
				\draw[->,line width =5pt,color=red!60] (-1.3,2.2) -- (-2, 1.5);
				
				\node at(1,0.5) {{\scriptsize$T(s') = {\cal F}_V(\Tr_c(s'))$}};
				\node at(2,1) {{\scriptsize$T(s_0) \text{ } \wedge \bigwedge_{s\in S}\ALL \GLOBAL\left(
						T(s) \rto
						\bigwedge_{(s,s')\in R}
						\EXIST \NEXT T(s')
						\wedge
				...
				\node at(-1, -0.6) {{\scriptsize${\cal F}_V(\Tr_{k+1}(s)) = \bigwedge_{(s,s')\in R}
						...
						\wedge 
						\ALL \NEXT \bigg( \bigvee_{(s,s')\in R} {\cal F}_V(\Tr_k(s')) \bigg) \wedge {\cal F}_V(\Tr_0(s))$ （$k\ge 0$）}};
