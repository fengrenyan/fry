	...
	%	\textcolor{lightblue}{
		\begin{center}
			\heiti\zihao{5}{
				...
				合作导师：\uwave{\hbox to 50mm{~~~~~~~~~~Erman Acar\footnote[frame]{University of Amsterdam, The Netherlands}~~~~~~~~~~~~~~~~~}} \\
...
%			\draw[->,line width =5pt,color=red!60] (-1.3,2.2) -- (-2, 1.5);
			...
			\draw[dashed, color=yellow,fill=yellow!40,opacity=0.2] (-7.4,0) rectangle (5.4,-1);
				...
				\node[rectangle,rounded corners =4pt,minimum width =40pt, minimum height =15pt,draw=yellow,fill=green!50!yellow]  at(-4,1.1)[draw,align=center] {{\scriptsize  (1) 只考虑$V$中的原子命题；}\\ {\scriptsize (2) 突出了树节点的内容，即：对于任意原子命题$p\in V$，若$p$在节点} \\ {\scriptsize 的标签中，则其正出现在特征公式中，否则负出现在特征公式中；}\\
