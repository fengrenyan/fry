...
%	}
	...
	\begin{frame}
		\frametitle{~国内外研究现状}
		\begin{figure}
			\includegraphics[scale=0.45]{figures/history3}
		\end{figure}
	\end{frame}
...
%	{\footnotesize  
		\begin{definition}[$\CTL$的语法]
			带索引的$\CTL$公式\emph{存在范式(existential normal form, ENF)}可以用巴科斯范式递归定义如下：
			\begin{align*}
			...
			\begin{figure}
				...
				\begin{tikzpicture}[scale=0.75]
					...
					\begin{minipage}{.2\textwidth}
						...
						\node[blueCircle2] (s22) at(2,-2) {$s_2'$};
						\draw[->] (s20) -- (s21);
						\draw[->] (s21) -- (s22);
...
%		\end{example}
		...
		\begin{proposition}\label{pro:uniforget}
			$\CTLforget(x\land \varphi(p,x), \{x\})$ 在 $\CTL$中是不可表示的。其中，$\varphi(p,x)$\footnote{$\varphi(p,x)$ 表示具有原子命题集$\Var(\varphi)=\{p,x\}$的公式。}为下面公式的合取~\cite{Maksimova:JANCL:1991}：
			\begin{align*}
				&\ALL\GLOBAL(\neg x \wedge \neg \ALL \GLOBAL p \rto \neg \ALL \NEXT \neg x),
				\qquad \ALL\GLOBAL(\neg \ALL\NEXT \neg x \rto \ALL \NEXT x),\\
				& \ALL\GLOBAL(\ALL\NEXT x \rto \neg x \wedge \neg \ALL \GLOBAL p),
