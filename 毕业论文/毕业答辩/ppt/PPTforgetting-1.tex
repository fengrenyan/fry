...
%	\setbeamertemplate{lem}[numbered]   
	...
	\begin{frame}
		\titlepage
%		{\Huge
		\frametitle{~~~~~~~~~~~~~~~2018级计算机科学与技术学院博士学位论文答辩}
%		\textcolor{blue}{
		\begin{center}
			\heiti\zihao{5}{
...
%		\node at(0,0)[draw=red,align=center]  {用人工智能知识表示与推理技术（遗忘），\\实现反应式系统（负例生成、系统精化等）\\形式化验证（最弱充分条件和更新等）的有效计算。};
		\node at(0,0)[draw=red,align=center]  {用人工智能知识表示与推理技术（遗忘），\\实现反应式系统最弱充分条件和更新等的有效计算，\\从而实现反应式系统验证中的负例生成、系统精化等。};
	\end{tikzpicture}
\end{figure}
			...
			\begin{figure}
				...
				\begin{tikzpicture}[scale=0.75]
					...
					\begin{minipage}{.2\textwidth}
						...
						\node[blueCircle] (s21) at(3,-2) {$s_1$};
						\node[blueCircle1] (s22) at(2,-2) {$s_2'$};
						\draw[->] (s20) -- (s21);
						\draw[->] (s21) -- (s22);
