...
\subsection{研究目标}
	\begin{frame}
		...
		\end{figure}
{\footnotesize
	用人工智能只是表示与推理技术（遗忘），实现反应式系统（负例生成、系统精化等）形式化验证（最弱充分条件和更新等）的有效计算。
...
\subsection{CTL下关键定义和性质}
\begin{frame}
	\frametitle{~研究内容（一）：CTL下关键定义和性质}
	{\scriptsize
...
%	\end{tikzpicture}
	\includegraphics[scale=0.27]{figures/sncAndWsc0}
...
\begin{figure} 
	\includegraphics[scale=0.27]{figures/sncAndWsc11}
	%\includegraphics[scale=0.27]{figures/applicationF4}
	%	\caption{进程的三种基本状态及其转换}
\end{figure} 
\end{frame}
...
\begin{frame}
	...
	\begin{figure} 
		\includegraphics[scale=0.27]{figures/sncAndWsc21}
		%\includegraphics[scale=0.27]{figures/applicationF4}
		%	\caption{进程的三种基本状态及其转换}
	\end{figure}
	\pause
	...
	\begin{figure}
		...
		\begin{tikzpicture}[scale=0.8]
			...
			\draw[dashed, color=yellow,fill=yellow!40,opacity=0.2] (-5.8,0) rectangle (3.8,-1);
				...
				\node[rectangle,rounded corners =4pt,minimum width =40pt, minimum height =15pt,draw=yellow,fill=green!50!yellow]  at(2,1.1)[draw,align=center] {{\scriptsize  只考虑$V$中的原子命题；}\\ {\scriptsize 突出了树节点的内容，即：对于任意原子命题$p\in V$，若$p$在节点} \\ {\scriptsize 的标签中，则其正出现在特征公式中，否则负出现在特征公式中；}\\
				{\scriptsize 公式中的时序算子表示了状态之间的转换关系。}};
...
\begin{frame}
	...
	{\footnotesize 
		...
		\begin{block}{遗忘封闭性}
			从$\varphi$中遗忘$V$中的元素得到的结果为：
			\begin{equation*}
				\bigvee_{{\cal K}\in  \{{\cal K}'\mid \exists {\cal K}''\in\Mod(\phi),\ {\cal K}''\lrto_V{\cal K}'\}} \textcolor{blue}{{\cal F}_{\overline V}({\cal K})}.
			\end{equation*}
		\end{block}
