	...
	\only<2>{
		\begin{itemize}
			...
			\begin{itemize}
				...
				\begin{itemize}
					\item 安全属性（Safety properties）：something
					bad never happens. ($\ALL \GLOBAL \neg \varphi$)
					\item 活性属性（Liveness	properties）：something good will eventually happen. ($\ALL \GLOBAL \ALL \FUTURE \varphi$)
					\item 持续属性（Persistence properties）：eventually for ever a certain proposition holds. ($\ALL \FUTURE \ALL \GLOBAL \varphi$)
					\item 公平属性（Fairness properties）：does,
					under certain conditions, an event occur repeatedly? （一种约束：在约束$fair = \GLOBAL \FUTURE a$下$\ALL \GLOBAL(a\rto \ALL \FUTURE b)$是否成立？）
				\end{itemize} 
			\end{itemize}
...
\begin{frame}
	...
	\only<2>{
		\begin{columns}
			...
			\column{0.3\textwidth}
			{\tiny $rei(\{\alpha_i\mid 1\le i\le n\})$、
