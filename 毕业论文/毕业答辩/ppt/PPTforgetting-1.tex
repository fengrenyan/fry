...
\documentclass[aspectratio=1610, 9pt, CJK]{beamer}
...
% \newtheorem{defn}{Definition}[section]
 ...
 \setbeamerfont{section in sidebar}{size=\fontsize{5}{4.5}\selectfont}
 \setbeamerfont{subsection in sidebar}{size=\fontsize{4}{3.5}\selectfont}
...
%	\setbeamertemplate{lem}[numbered]   
	...
	\begin{frame}
		\titlepage
%		{\Huge
		\frametitle{~~~~~~~~~~~~~~~~计算机科学与技术学院博士学位论文答辩}
%		\textcolor{blue}{
		\begin{center}
			\heiti\zihao{5}{
				姓\quad \ \ \ 名：\underline{\hbox to 50mm{~~~~~~~~~~~~~~~~~冯\ 仁\ 艳~~~~~~~~~~~~~~~~}}\\
				导\quad\ \ \ 师：\underline{\hbox to 50mm{~~~~~~~~~~~~~~~~~王\ 以\ 松~~~~~~~~~~~~~~~~}}\\
				合作导师：\underline{\hbox to 50mm{~~~~~~~~~~~~~~~~~Erman Acar~~~~~~~~~~~~~~~~}} \\
			%	学科专业：\uwave{\hbox to 50mm{~~~~~~~~~~~~~~软件工程~~~~~~~~~~~~~~~~~~}}\\
				研究方向：\underline{\hbox to 50mm{~~~~~~软件工程技术与人工智能~~~~~~~~~~~~~~~~~~~}}
			}
		\end{center}
%	}
	\end{frame}
	...
	\begin{frame}  
		\frametitle{研究背景和意义：{\footnotesize 系统正确对国防、太空勘测和交通运输至关重要}}
			\begin{figure}
				...
				\centering
				\subfigure{
				\includegraphics[scale=0.23]{figures/soft1}}\qquad
				%\subfigure{\includegraphics[scale=0.4]{figures/soft2}}
			  \subfigure{\includegraphics[scale=0.17]{figures/soft3}}\qquad
...
%			\end{itemize}
	\begin{figure}
		...
		\begin{tikzpicture}[scale=0.75]
			...
			\node[blueCircle] (s1) at(0,0) {$s_1$};
			\node[blueCircle] (s2) at(-1,0) {$s_2$};
...
%		{\footnotesize 	
			...
			SNC和WSC分别用于描述给定理论下的最一般的结果（consequence）和最一般的诱因（abduction）\cite{DBLP:journals/ai/Lin01}。 
		满足下面两个条件的$\psi$称为$q$在理论$\Sigma$下的WSC:
		\begin{itemize}
...
%				 在一阶逻辑中，从公式$\varphi$中遗忘掉一个$n$元谓词$P$的结果是$\exists R.\varphi[P/R]$，即将公式$\varphi$中的所有$P$的出现都用一个新的$n$元谓词$R$来替代。
				...
				\column{0.5\textwidth}
				\begin{figure}
					\includegraphics[scale=0.4]{figures/forgetting1}
				\end{figure}
			\end{columns}
		\end{block}}
	\only<2>{
		\begin{figure}
			\includegraphics[scale=0.45]{figures/CTLmu}
		\end{figure}
%		\begin{itemize}
...
\subsection{研究目标}
	\begin{frame}
		\frametitle{~研究目标}
		\begin{figure}
			\includegraphics[scale=0.45]{figures/mubiao}
		%	\caption{文章组织结构示意图}
		\end{figure}
%{\footnotesize
\begin{figure}
	\begin{tikzpicture}
%		\node at(0,0)[draw=red,align=center]  {用人工智能知识表示与推理技术（遗忘），\\实现反应式系统（负例生成、系统精化等）\\形式化验证（最弱充分条件和更新等）的有效计算。};
		\node at(0,0)[draw=blue!50,align=center,fill=blue!30]  {用人工智能知识表示与推理技术（遗忘），\\实现反应式系统最弱充分条件和更新等的有效计算，\\从而实现反应式系统验证中的负例生成、系统精化等。};
	\end{tikzpicture}
\end{figure}
	
%		通过人工智能的知识表示与推理(KR)技术，从{\em 遗忘理论}出发，研究{\em 反应式系统}(在某个符号集上)SNC和WSC的表示与计算，具体为：
%		\begin{itemize}
%			\item CTL和$\mu$-演算遗忘理论框架；
...
%			\uncover<1->{~~~本论文研究反应式系统下，CTL和$\mu$-演算的遗忘理论，并使用遗忘计算WSC。具体为：}
%			\begin{itemize}
%				\vskip 8pt
%				\item<2-> CTL和$\mu$-演算的遗忘理论
%				\begin{itemize}
%					\item CTL的遗忘理论
%					\item $\mu$-演算的遗忘理论
%				\end{itemize}
...
%				\EXIST_{\tuple{ind}}(\phi \UNTILL \phi)  
			\end{align*}
			其中，$p$为原子命题集$\Ha$中的元素。%，$ind \in \Ind$。
...
%		${\cal V}$：变元符号的可数集。 
		\begin{definition}[$\mu$-演算公式]
			...
			\[
			\varphi ::=   p\mid  \textcolor{red}{X} \mid \neg \varphi\mid \varphi \vee \varphi \mid \ALL\NEXT \varphi\mid  \textcolor{red}{\nu X. \varphi}.
			\]
		%	其中$p\in \Ha$且$X\in {\cal V}$。
		\end{definition}
		\begin{definition}[$\mu$-演算的语义]
			给定$\mu$-演算公式$\varphi$、Kripke结构{\em $\Hm=(S,R,L,r)$}和一个从${\cal V}$中的变量到$\Hm$中状态的赋值函数\textcolor{red}{$v: {\cal V} \rto 2^S$}。公式在$\Hm$和$v$上的解释是$S$的一个子集$\left\| \varphi\right\|_v^{\Hm}$（如果在上下文中$\Hm$是明确的，则可以省去上标）：
%			\begin{columns}
%				\column{0.5\textwidth}
				\begin{align*}
			...
			\begin{figure}
				...
				\begin{tikzpicture}[scale=0.75]
					...
					\begin{minipage}{.2\textwidth}
						...
						\node[blueCircle] (s11) at(3,1) {$s_1$};
						\node[blueCircle1] (s12) at(2,1) {$s_2$};
					...
					\end{minipage}
					\begin{minipage}{.2\textwidth}
						\node[blueCircle] (s20) at(3,-1) {$t_0$};
						\node[blueCircle] (s21) at(3,-2) {$t_1$};
						\node[blueCircle1] (s22) at(2,-2) {$t_2'$};
						\draw[->] (s20) -- (s21);
						\draw[->] (s21) -- (s22);
...
%			\end{align*}
%			Maksimova证明了$\varphi(p,x)\land \varphi(p,y)\models x\lrto y$，且不存在CTL公式 $\psi$使得 $\Var(\psi)=\{p\}$且$\varphi(p,x)$ $\models x\lrto \psi$，即CTL不具有Beth性质。
%		\end{example}
...
%%	\only<2,3>{
%		\begin{proposition}\label{pro:uniforget}
%			$\CTLforget(x\land \varphi(p,x), \{x\})$ 在 CTL中是不可表示的。
%		\end{proposition}
...
%	{\footnotesize
	\begin{columns}
		...
		\begin{itemize}
			...
			\item 初始Kripke结构的特征公式看作$\CTL$公式——${\cal F}_{\Ha}(\Hm)$；
		\end{itemize}
	\column{0.5\textwidth}
	\begin{figure}
		\centering
...
%		 \end{figure}
	\end{columns}
%} 
{\noindent}	 \rule[-10pt]{17.5cm}{0.1em}\\
\begin{figure} 
%	\begin{tikzpicture}
%		\tikzstyle{every node}=[font=\small,scale=0.8]
%		\node[rectangle,rounded corners =5pt,minimum width =50pt, minimum height =20pt,draw=orange,fill=orange] (s0) at(0,0) {\textcolor{white}{遗忘}};
%		\node[rectangle,rounded corners =5pt,minimum width =50pt, minimum height =20pt,draw=blue!60,fill=blue!60] (s1) at(2,0) {\textcolor{white}{知识更新}};
%		\node at(0.5,1)[draw=blue!40,align=center] {Katsuno和Mendelzon\\ 基本条件(U1)-(U8)};
%		%\draw[dashed, color=blue!20] (0,1.2) rectangle (3,0.5);
%		\node at(0,-1)[draw=blue!40,align=center] {任意公式的SNC\\（WSC）都可计算};
%		
%		\node[rectangle,rounded corners =10pt,minimum width =50pt, minimum height =20pt,draw=orange!60,fill=orange!40] (s0) at(-2,2) {\textcolor{white}{SNC（最强必要条）}};
%		\node[rectangle,rounded corners =10pt,minimum width =50pt, minimum height =20pt,draw=orange!60,fill=orange!40] (s0) at(-2,-2) {\textcolor{white}{WSC（最弱充分条件）}};
%		
%	
%		
%	%	\node[color=orange] at(-1, 5.2) {遗忘总是CTL可表示的};
%	\end{tikzpicture}
...
\centering{投稿在KR 2020、AMAI和INS上的文章都有对反应式系统最弱充分条件的讨论。}
\end{frame}
...
%				\item 任意$\Ha$上的有限初始结构$\Hm$（为了简化符号，用初始Kripke结构$\Hm$代替初始结构$(\Hm,s_0)$）都能用一个CTL公式——\fbox{特征公式${\cal F}_{\Ha}(\Hm)$}来表示；
...
\begin{frame}
	\frametitle{研究内容（三）（1）基于模型的CTL遗忘}
	%	\begin{figure}
	...
	\begin{figure}
		...
		\begin{tikzpicture}%[scale=0.8]
%			\tikzstyle{every node}=[font=\small,scale=0.8]
			\node[rectangle,rounded corners =10pt,minimum width =50pt, minimum height =20pt,draw=blue,fill=blue!40] (s0) at(-1,6) {基于模型的有界$\CTL$遗忘};
			\node[color=orange] at(-1, 5.2) {遗忘总是$\CTL$可表示的（KR 2020）};
			...
			\draw[->] (s2) -- (s3);
			\draw[->] (s2) -- (s4);
			\draw[<->,color=gray] (s4) -- (s5);
			\draw[->] (s4) -- (2,2)--(s3);
			\draw[->,color=gray] (s5) -- (3,0) -- (s6);
			\draw[->] (s3) -- (3,3) -- (s6);
			%			\node[blueCircle] (s0) at(0,1) {{\tiny 就绪}};
			%			\node[blueCircle1] (s1) at(0.8,-0.2) {{\tiny 执行}};
			%			\node[redCircle] (s2) at(-0.8,-0.2) {{\tiny 阻塞}};  
	...
	\begin{figure}
		...
		\begin{tikzpicture}%[scale=0.8]
			...
			\node[color=red] at(0,4) {$
					...
					\end{array}
					\right.$};
			\draw[dashed, color=yellow,fill=yellow!40,opacity=0.2] (-5.5,4.5) rectangle (5,3.5);
	...
	\begin{figure}
		...
		%	\tikzstyle{every node}=[font=\small,scale=0.8]
			...
			\node at(0,4) {$
					...
					\end{array}
					\right.$};
			\draw[dashed, color=orange] (-5.5,4.5) rectangle (5,3.5);
...
%			\draw[->,line width =5pt,color=red!60] (-1.3,2.2) -- (-2, 1.5);
			...
			\draw[dashed, color=yellow,fill=yellow!40,opacity=0.2] (-7.4,0) rectangle (5.4,-1);
				...
				\node[rectangle,rounded corners =4pt,minimum width =40pt, minimum height =15pt,draw=orange,fill=orange!60]  at(-4,1.1)[draw,align=center] {{\scriptsize  (1) 只考虑$V$中的原子命题；}\\ {\scriptsize (2) 突出了树节点的内容，即：对于任意原子命题$p\in V$，若$p$在节点} \\ {\scriptsize 的标签中，则其正出现在特征公式中，否则负出现在特征公式中；}\\
				{\scriptsize (3) 公式中的时序算子表示了状态之间的转换关系。}};
...
%			\draw[->,line width =5pt,color=red!60] (-1.3,2.2) -- (-2, 1.5);
			...
			\node[color=red] at(-3,0.5) {$T(s') = {\cal F}_V(\Tr_c(s'))$};
			\node[color=red] at(0,1) {$T(s_0) \text{ } \wedge \bigwedge_{s\in S}\ALL \GLOBAL\left(
					T(s) \rto
					\bigwedge_{(s,s')\in R}
					\EXIST \NEXT T(s')
					\wedge
			...
			\draw[dashed, color=yellow,fill=yellow!40,opacity=0.2] (-5,1.5) rectangle (5,0.3);
			
			\node at(-4.5,-0.2) {$
				{\cal F}_V(\Tr_0(s)) =  \bigwedge_{p \in V\cap L(s)}p
				\wedge \bigwedge_{q\in V-L(s)} \neg q,$  
			};
			\node at(-1, -0.6) {${\cal F}_V(\Tr_{k+1}(s)) = \bigwedge_{(s,s')\in R}
				\EXIST \NEXT {\cal F}_V(\Tr_k(s')) 
				\wedge  
				\ALL \NEXT \bigg( \bigvee_{(s,s')\in R} {\cal F}_V(\Tr_k(s')) \bigg) \wedge {\cal F}_V(\Tr_0(s))$ （$k\ge 0$）}; %计算树特征公式
		%	\draw[dashed, draw=orange] (-7.4,0) rectangle (5.4,-1);
			...
			\draw[->] (s4) -- (2,2)--(s3);
			\draw[->,dashed,color=gray] (s5) -- (3,0) -- (s6);
			\draw[->] (s3) -- (3,3) -- (s6);
			%			\node[blueCircle] (s0) at(0,1) {{\tiny 就绪}};
			%			\node[blueCircle1] (s1) at(0.8,-0.2) {{\tiny 执行}};
			%			\node[redCircle] (s2) at(-0.8,-0.2) {{\tiny 阻塞}};  
...
%		\begin{tikzpicture}[scale=0.8]
%			\tikzstyle{every node}=[font=\small,scale=0.8]
%			\node[rectangle,rounded corners =10pt,minimum width =50pt, minimum height =20pt,draw=blue,fill=blue!40] (s0) at(-1,6) {基于模型的有界CTL计算方法};
%			\node[color=orange] at(-1, 5.2) {遗忘总是CTL可表示的};
...
%			
%			
%			\node[rectangle,rounded corners =4pt,minimum width =40pt, minimum height =15pt,draw=blue,fill=blue!20] (s1) at(-2,3) {{\scriptsize 有界CTL}};
%			
%			\node[rectangle, minimum width =50pt, minimum height =15pt,draw=blue,fill=blue!10] (t1) at(-5,3.5) {{\scriptsize 构成公式的原子命题集有限}};
%			
%			\node[rectangle, minimum width =50pt, minimum height =15pt,draw=blue,fill=blue!10] (t2) at(-5.5,2.5) {{\scriptsize 初始结构的状态空间${\cal S}=\{b_1,\dots, b_m\}$}};
...
%		\begin{proposition} 
%			给定CTL公式$\varphi$和原子命题集$V \subseteq \Ha$。
...
\subsection{研究内容（三）（3）算法CTL-forget实验} 
%\begin{frame}
%	\frametitle{基于归结的算法CTL-forget实现}
%	{\footnotesize
%		\begin{block}{系统描述}
%			\begin{itemize}
%				\item 输入输出：基于Prolog的CTL-forget算法实现系统以CTL公式和原子命题集为输入，CTL公式为输出；
%				\item 系统识别的CTL公式的符号与第\ref{chapter02}章中CTL的语言符号对应关系如下：
%				\begin{itemize}
...
%	{\footnotesize
%		(1)	标准数据集来源于CTL-RP： \url{https://sourceforge.net/projects/ctlrp/}
%		\begin{figure}
%			\includegraphics[scale=0.35]{figures/expTab}
%%			\caption{
%%				标准数据集来源于CTL-RP：\\ \url{https://sourceforge.net/projects/ctlrp/}}
%		\end{figure}
%%		\begin{block}{实验结果}
%%			标准数据集来源于CTL-RP\footnote{\url{https://sourceforge.net/projects/ctlrp/}}
...
%			\begin{itemize}
%				\item CTL的遗忘理论：基本性质（表达性理论、代数属性和封闭性等）
...
\begin{frame} 
	...
	{\footnotesize 
		\only<1>{\begin{block}{$\CTLsnf$子句}
				具有下面几种形式的公式称为CTL全局子句分离范式（separated normal form with global clauses for \CTL，$\CTLsnf$子句）\cite{zhang2008first,zhang2014resolution}：
				\[
				\begin{array}{ll}
		...
		\only<2>{
			\begin{block}{转换规则}
				一个CTL公式$\varphi$可以通过下表中的规则转换为一个$\CTLsnf$子句集，记为$T_{\varphi}$。 
				{\tiny
					\begin{table}%[width=.9\linewidth,cols=4,pos=h] 
						\centering 
				...
				$q$是一个原子命题, $l$是一个文字, $D$是文字的析取（即子句）, $p$是新的原子命题；$\varphi$，$\varphi_1$，和$\varphi_2$都是CTL公式。
			\end{block}
		} 
	}
\end{frame}
...
\begin{frame} 
	...
	{\scriptsize 
		...
		%			令$V\subseteq \Ha$是原子命题集，${\cal K}_i$ $(i=1,2)$是两个具有$V$-互模拟关系的$\Ind$-结构，即：${\cal K}_1 \lrto_V {\cal K}_2$。若$\Phi$是一个CTL公式且$\IR(\Phi, V)$，则有${\cal K}_1\models \Phi$当且仅当${\cal K}_2\models \Phi$。
		%		\end{theorem}
		%	}
	}
\end{frame}
...
\begin{frame}
	...
	{\footnotesize 
		...
		\end{definition}
		\only<1>{\begin{block}{遗忘理论公设}
				给定CTL公式$\varphi$、$\varphi'=\CTLforget(\varphi, V)$、原子命题集$V\subseteq \Ha$和$\varphi'=\CTLforget(\varphi, V)$，CTL下遗忘理论公设如下：
				\begin{itemize}
					\item[(\W)] 削弱：$\varphi \models \varphi'$；
					\item[(\PP)] 正支持：对任意与$V$无关的公式$\eta$，若$\varphi \models \eta$则$\varphi' \models \eta$；
					\item[(\NgP)] 负支持：对任意与$V$无关的公式$\eta$，若$\varphi \not \models \eta$则$\varphi' \not \models \eta$；
...
%	 \begin{block}{约束}
%		\begin{itemize}
%			\item[(1)] 出现在CTL公式中的原子命题的个数是有限的（即$\Ha$是有限的）；
%			\item[(2)] 初始结构的状态空间$S$是一个有限的固定状态空间${\cal S}=\{b_1,\ldots,b_m\}$的子集（即$S \subseteq {\cal S}$），且使得对于任意约束长度的CTL公式$\varphi$，若$\varphi$是可满足的，则存在一个状态空间是${\cal S}$的子集的初始结构$(\Hm,s_0)$，使得$(\Hm,s_0)\models \varphi$。
%		\end{itemize}
%	\end{block} 
%\end{frame}
	...
	{\footnotesize
		...
		\only<2>{	\begin{block}{记号}
			...
			\begin{proposition}\label{pro:resEQ}
				令 $\varphi$ 为一个 CTL公式，$V\subseteq\cal A$为原子命题集。则
...
\begin{frame}
	...
	{\footnotesize
		\only<1>{	
			...
			\end{block} 
			\begin{lemma} 
				如果$j\in {\cal I}$，$\psi_i,\varphi_i~(1\le i\le n)$为 CTL公式，那么：
				\begin{itemize}
	...
	{\footnotesize
		...
		\only<2>{
			...
			\begin{proposition}[遗忘存在的子类]\label{pro:fogCTL}
				给定CTL公式$\varphi$，若$\varphi$满足下面约束：
