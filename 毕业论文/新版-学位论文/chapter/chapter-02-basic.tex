\chapter{Kripke结构、时序逻辑以及遗忘理论}\label{chapter02}
...
给一个可数无限索引的集合$\Ind$，一个初始$\Ind$-Kripke结构是一个五元组$\Hm=(S,R,L,[\_],s_0)$，其中：
\begin{itemize}
...
从初始$\Ind$-Kripke结构$\Hm$中同时去掉$[\_]$和$s_0$得到的结构称为Kripke结构。
...
通常一个转换系统（transition system）能够被抽象为一个Kripke结构\cite{Baier:PMC:2008}。
...
\begin{itemize}
	...
	\item 标点符号：“(”和“)”。
\end{itemize}
带索引的$\CTL$公式（简称公式）的时序算子与$\CTL$公式的时序算子相同，是路径量词和时序操作符的组合（路径量词在前，时序操作符在后），如：$\ALL \NEXT$、$\EXIST \NEXT$、$\EXIST_{ind} \NEXT$、 $\ALL \FUTURE$等。
...
与经典命题逻辑一样，给联结符号规定优先级，有时候会带来意想不到的方便。带索引的$\CTL$中的联结符号的优先级如下序列所示，每个左边的联结符号优先于右边的联结符号：
\begin{align*}
...
\begin{definition}[带索引的$\CTL$的语义]\label{def:ctl:semantic}
	给定带索引的$\CTL$公式$\varphi$，初始$\Ind$-结构$\Hm=(S,R,L,[\_],s_0)$和状态$s\in S$。$(\Hm,s)$与$\varphi$之间的可满足关系$(\Hm,s)\models \varphi$定义如下：
	\begin{itemize}
...
与Browne和Bolotov等人的工作类似，本文只将初始$\Ind$-结构作为模型的候选项\cite{browne1988characterizing,Bolotov:1999:JETAI}。换句话说，对于给定的$\Ind$-结构$(\Hm,s)$和带索引的$\CTL$公式$\varphi$，如果$(\Hm,s)\models \varphi$且$s = s_0$，则称$(\Hm,s)$为公式$\varphi$的一个\emph{模型}。更清楚地说，对于给定的初始$\Ind$-结构${\cal K}=(\Hm,s_0)$，如果${\cal K} \models \varphi$，则称$\cal K$是$\varphi$的一个模型。
...
\end{lemma}
\begin{proof}
	(i) $(\Rto)$ For each model $(\Hm, s)$ of $\ALL\GLOBAL (\varphi\land\psi)$\\
	$\Rto$ $\forall \pi=(s=s_0, s_1, \dots)$ there is $(\Hm,s_i) \models \varphi \wedge \psi$ for all $i\geq 0$\\
	$\Rto$ $\forall \pi=(s=s_0, s_1, \dots)$ there is $(\Hm,s_i) \models \varphi$ and $(\Hm,s_i) \models \psi$ for all $i\geq 0$\\
	$\Rto$ $(\Hm,s) \models (\ALL\GLOBAL \varphi) \land (\ALL\GLOBAL\psi)$.
	
	$(\Lto)$ For each model $(\Hm,s)$ of $(\ALL\GLOBAL \varphi) \land (\ALL\GLOBAL\psi)$\\
	$\Rto$ $\forall \pi=(s=s_0, s_1, \dots)$ there is $(\Hm,s_i) \models \varphi$ and $(\Hm,s_i) \models \psi$ for all $i\geq 0$\\
	$\Rto$ $\forall \pi=(s=s_0, s_1, \dots)$ there is $(\Hm,s_i) \models \varphi \wedge \psi$ for all $i\geq 0$\\
	$\Rto$ $(\Hm, s) \models \ALL\GLOBAL (\varphi\land\psi)$.
	
	(ii) $(\Rto)$ For each model $(\Hm, s)$ of $\ALL\GLOBAL \ALL\GLOBAL (\varphi)$\\
	$\Rto$ $\forall \pi=(s=s_0, s_1, \dots)$ there is $(\Hm,s_i) \models \ALL\GLOBAL (\varphi)$ for all $i\geq 0$\\
	$\Rto$ $(\Hm,s) \models \ALL\GLOBAL (\varphi)$.
%	$\Rto$ $\forall \pi=(s_i, s_{i+1}, \dots)$ there is $(\Hm,s_j) \models \varphi$ for all $i\geq 0$ and $j \geq i$

	$(\Lto)$ For each model $(\Hm, s)$ of $\ALL\GLOBAL (\varphi)$\\
	$\Rto$ $\forall \pi=(s=s_0, s_1, \dots)$ there is $(\Hm,s_i) \models \varphi$ for all $i\geq 0$\\
	$\Rto$ $\forall \pi'=(s_i, s_{i+1}, \dots)$ there is $(\Hm,s_j) \models \varphi$ for all $i\geq 0$ and $j \geq i$\\
	$\Rto$ $\forall \pi=(s=s_0, s_1, \dots)$ there is $(\Hm,s_i) \models \ALL\GLOBAL (\varphi)$ for all $i\geq 0$\\ 
	$\Rto$ $(\Hm,s) \models\ALL\GLOBAL \ALL\GLOBAL (\varphi)$.
	
	(iii) $(\Rto)$ For each model $(\Hm,s)$ of $\ALL\GLOBAL (\start \rto\varphi)$\\
	$\Rto$ $\forall \pi=(s=s_0, s_1, \dots)$ there is $(\Hm,s_i) \models \start \rto\varphi$ for all $i\geq 0$\\
	$\Rto$ $(\Hm,s) \models \varphi$ since $(\Hm,s) \models \start$
	
	$(\Lto)$ For each model $(\Hm, s)$ of $\varphi$\\
	$\Rto$ $(\Hm,s) \models \start\rto \varphi$ since $(\Hm,s) \models \start$\\
	$\Rto$ $(\Hm,s') \models \start\rto \varphi$ since $(\Hm,s') \not \models \start$ for any $s'\not = s$\\
	$\Rto$ $\forall \pi=(s=s_0, s_1, \dots)$ there is $(\Hm,s_i) \models \start \rto\varphi$ for all $i\geq 0$\\ 
...
\begin{lemma}\label{lem:indno}
	...
	\begin{itemize}
		\item[(i)] $\varphi$ 和 $\varphi'$ 是等价可满足的，其中$\varphi'$ 是由 $\varphi$通过用新的索引命名现有的一些索引得到的公式，即$\varphi$是将出现在其中的一些索引用不出现在其中的索引全部替换得到，不同的索引对应不同新索引。
		\item[(ii)] $\varphi\models\varphi''$，其中 $\varphi''$ 是移除掉 $\varphi$中的索引得到的公式。
	\end{itemize}
\end{lemma}
\begin{proof}
	(i) Without loss of generality, let $\varphi'=\varphi[i/j]$ be the formula obtained from $\varphi$ by renaming index $i$ with a fresh index $j$.
	
	 $(\Rto)$ For each model $(\Hm,s)$ of $\varphi$ with $\Hm = (S,R,L,[\_], s)$ \\
	 $\Rto$ $(\Hm',s) \lrto_{\emptyset} (\Hm,s)$ and $(\Hm',s) \models \varphi'$, where $\Hm' = (S,R,L,[\_]',s)$ and for each $x \in \Ind$
	 \[[x]'=
	 \left\{
	 \begin{array}{ll}
	 	[i], \qquad \hbox{$x = j$;} \\
	 ...
	 We can similarly prove that for each model $(\Hm,s)$ of $\varphi'$, there is $(\Hm',s)$ s.t. $(\Hm',s) \lrto_{\emptyset} (\Hm,s)$ and $(\Hm',s) \models \varphi$.
	 
	 (i) It is evident from the semantics of the indexed $\CTL$ formula.
\end{proof}
...
接下来也将$\CTLsnf(\varphi)$记为由$\varphi$通过转换规则获得的 $\CTLsnf$ 子句的集合。
因此，下面的引理是显然的。
...
\end{lemma}
\begin{proof}
	(i) It is evident that $\textbf{Trans(1)}$ and $\textbf{Trans(2)}$ introduce a fresh index for each occurrence of the path quantifier $\EXIST$. Moreover, once the path quantifier $\EXIST$ is labeled by an index, it will not be labeled by another index.
	%According to the transformation rules in Table~\ref{tab:trans},  
	
	(ii) We know that there is an exact one fresh index for each occurrence of the path quantifier $\EXIST$ by (i). Moreover, there will not produce new $\EXIST$-sometime clauses in the derivation.
	%Therefore, (ii) holds.
	
	(iii) It is obvious from the transformation and resolution rules.
\end{proof}
...
给定两个模态子句$C=C_0 \vee \MPK C_1 \vee \dots \vee \MPK C_{n-1} \vee \MPB C_n$和$C'=C_0' \vee \MPK C_1' \vee \dots \vee \MPK C_{m-1}' \vee \MPB C_m'$，如果下面三个条件满足，则说$C$包蕴$C'$：
\begin{itemize}
...
令$\phi=C_0 \vee \MPK C_1 \vee \dots \vee \MPK C_{n-1} \vee \MPB C_n$为模态子句，$\phi$在$V$上的清楚操作也记为$Supp(V,\phi)$，且：
...
\begin{theorem}[定理4.1~\cite{feng2018strongest}]\label{thm:S5:SNC}
	...
	\end{itemize}
\end{theorem}
此时，由算法~\ref{alg:compute:s5:forgetting}不难得出计算SNC和WSC的算法，这里就不在赘述。
