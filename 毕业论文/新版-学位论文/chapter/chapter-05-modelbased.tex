...
本章只涉及有限的初始Kripke结构，因此，本章混用$\lrto_V$这个符号和$V$-互模拟这个概念。
...
\end{proposition}
\begin{proof}
假定每个状态或原子命题占据$\log m$的空间且$n\leq m$，则一个状态对$(s,s')$占据$2*\log m$位。对任意的$B \subseteq {\cal S}$ 且$s_0\in B$，构造如下初始Kripke结构$(\Hm,s_0)$，其中$\Hm = (B,R,L,s_0)$，则$R$中至多有$\frac{|B|^2}{2}$个状态对且$L$中至多$|B|*n$个$(s,A)$对（$A\subseteq \Ha$）。因此，$(\Hm,s_0)$至多占据$(|B|+|B|^2+|B|*n)*\log m$位。此外，对于状态集合$B$，初始状态有$|B|$种选择，有$|B|^{|B|}$种关系$R$，$(2^n)^{|B|}$种标签函数$L$。
	
	
	
	Let $k=n-x$, for any initial \MPK-structure ${\cal K}=(\Hm, s_0)$ with $i\geq 1$ nodes, in the worst, i.e., $ch(\Hm,V)=i$, we will spend $N(i)$ space to store the characterizing formula. Where
	\begin{align*}
		& N(i)=(k+(\dots + (k + 2ik)*(2i))\dots *(2i))\\
		& = (2i)^0k + 2ik + (2i)^2k+ \dots + (2i)^{(i-1)}k\\
		& = \frac{(2i)^i -1}{2i-1} k.
	\end{align*}
	In the worst case, i.e., there is $m^{m+2}*2^{nm}$ initial \MPK-structures with $m$ nodes, we will spent $m^{m+2}*2^{nm}*N(m)$ bytes to store the result of forgetting.
	...
	Therefore, the space complexity is $O((n-x)m^{2(m+1)}2^{nm})$ and the time complexity is at least the same as the space.
\end{proof}
...
dfjlasdjfleajf;jfojajfjsfdjas;lfja;s
