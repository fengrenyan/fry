...
1. 给出$\CTL$下的遗忘理论的概念及其相关性质。首先，本文从模型在某个原子命题集合上互模拟的角度给出了遗忘的定义；其次，本文探讨了遗忘算子的代数属性，包括模块性、交换性和同质性；第三，表达定理表明遗忘和Zhang等人提出的四条准则具有当且仅当的关系，即：遗忘的结果满足四条准则，且满足那四条准则的公式为遗忘的结果。

2. 提出一种基于归结的方法计算$\CTL$下的遗忘。该方法使用Zhang等人提出的归结系统，在这个过程中需要将$\CTL$公式转换为$\CTLsnf$（separate 。。。global clauses），最后再将具有索引的$\CTLsnf$子句转换为$\CTL$过程。在这一过程中总是保持互模拟等价关系。
