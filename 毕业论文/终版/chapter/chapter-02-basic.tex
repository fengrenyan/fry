...
此外，Emerson和 Halpern 证明了$\CTL$具有小模型属性：如果一个公式是可满足的，那么它在一个小的有限模型下是可满足的~\cite{DBLP:journals/jcss/EmersonH85}。
...
在给出$\mu$-演算析取公式的定义之前，事先给出$\mu$-公式的另一种定义，称为\emph{覆盖-语法}（cover-syntax）。该定义是用\emph{覆盖操作}（cover operator）集替换上述$\mu$-公式的定义中的$\EXIST\NEXT$得到。在覆盖-语法中，
\begin{itemize}
...
对任意一阶公式$\alpha$、关系符号集$P$ 和句子$Th$:
	...
	\end{itemize}
其中 $\overline{\Phi}$是出现在$Th\wedge\alpha $且不出现在$P$中的关系符号集。
\end{theorem}
