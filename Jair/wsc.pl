...
ctl2NNF(P, P) :- prop(P). %将CTL公式转换为NNF公式，暂时不考虑有符号出现再until模态词前面的情况。
ctl2NNF(- P, - P) :- prop(P).
...
tran2SNF(L, Ind, N, V, IndM, R) :- tran2SNF_list(L, V1, Ind, N, IndM1, NM, L1),  %Ind表示index的初始值0，IndM表示index的最大值（结束transform过程后的值）； N表示变量的初始值0，NM表示transform结束后引入的原子的个数-1.
...
satCall(Filename, P, PN, ResultF) :-
	...
	(SAT = false, write(Str,'UsedTime:'), write(Str, V1), nl(Str); 	
		...
		write(Str, 'LenSNF='), write(Str, LenRes), nl(Str),
...
newCall(Filename, P, PN, ResultF) :-
	...
	pick_props(P, PN1, P1), %随机从PN1中选择P个原子进行遗忘
	%difference_list(P, PN1, P1), %P is the atom of varphi and PN1 is the atom of formula
	write("P1="), write(P1), nl,
        write("T="), write(Formula), nl, 
	%add_propTerm(PN1),!,
	catch(time_output_1(new_ctlForget(Formula, P1, Res, Ack, R, Exist), U, V, W),E, false),
	...
	write(Str, 'Res===========: '), write(Str, Res), nl(Str), %write(Str,"\n"),
	write(Str, 'ArcM===========: '), write(Str, Ack), nl(Str), %write(Str,"\n"),
	write(Str, 'RRR: '), write(Str, R), nl(Str), %write(Str,"\n"),
	write(Str, 'Exist: '), write(Str, Exist), nl(Str), %write(Str,"\n"),
	write(Str,'...............................................'), nl(Str),
        retractall(prop(_)), retractall(pair(X,Y)),
	close(Str).
...
new_ctlForget(F, V, Res, Ack, R, Exist,LenSNF, LenRes) :- (gain_prop((F & q), P), ctl2NNF((F & q), F1), L=[start-> z, z -> F1], assert(prop(z)), 
	tran2SNF(L, 0, 0, V1, IndM, SNF), tran2SNFClt(SNF, LC), subtract(P, V, Vf), append(Vf, V1, V2), append([z], V2, V3), 
	write("\n =======IndexM ========\n"), write(IndM),
		  write("\n =======IndexM ========\n"),
 	length(LC, LenSNF), 
	...
	append(V3, W, V6),
	(L2=false, R = false, Res = false, LenRes=0, Ack = false, Exist = false; length(P, LenVarF), length(Vf, LenV), 
		...
		(R4 = false, R = false, Res = false, LenRes=0, Ack = false, Exist = false;
		 ...
		 instante(R44, Vf, V6, [], R5), write("\n =======Ins ========\n"), write(R5), %write(R44),
		 write("\n =======Ins ========\n"), 
		 subtract(V6, R5, R6), %fImp(TL2, R4, V6, R5,0),
		 append(Vf, R6, V7), removeAtom(R44,Vf,R55),  write("\n =======Remove ========\n"), write(R55),
		 write("\n =======Remove ========\n"), 
		 ...
		 ackerM(R55, AcV, Ack),
		  ...
		  write("\n =======ackerM ========\n"), write(Ack),
		  write("\n =======ackerM ========\n"),
		  retractall(pair(X,Y)), retractall(formula(X1,Y1,Z1)), retractall(prop(Z)), %在最后移除所有的pair和formula对，以便计算另外的公式
		 retractall(alpha(_,_,_)), retractall(beta(_,_,_)), retractall(snfclause(_, _, _, _)),
		  R = true, length(Ack, Len), subtract(V6, Vf, Vv), length(Vv, LenVv), (Len = LenVv, Exist = 1; Exist =0)
		 )
		)
	); 
...
ctlForget(F, V, R) :- (gain_prop(F, P), L=[start-> z, z -> F], assert(prop(z)), 
	...
	%ctlForget_list(SL, TL, W,V2, L2),
	append(V3, W, V6),
	(L2=false, R = false; append(V3, P, V4), sort(V4, V5), temp_resolution(TL, W, L2, V3, R1, TL1, W1, V5), 
		append(L2, LC, L3), append(L3, R1, R2), sort(R2, R3), split2ST(R3, SL2, TL2, W2, 0), step_resolution(SL2, V3, R4), %trace, 
		(R4 = false, R = false;
		 append(R4, TL2, Ry), subtract(Ry, [snf_clause(true,[true], [true], nil)], R44),
		 instante(R44, V, V6, [], R5), write("\n =======Ins ========\n"), write(R5),
		 write("\n =======Ins ========\n"), 
		 subtract(V6, R5, R6), fImp(TL2, R4, V6, R5,0),
		 append(V, R6, V7), removeAtom(R44,V7,R55),  write("\n =======Remove ========\n"), write(R55),
		  write("\n =======Remove ========\n"), 
		 insAllL(R44, V, V6, R5), base(O0), re2CTL(V, FR), removeSing(R55, R),
		 append([z], V1, V11), nestFormula(FR, V11, FRR),
		 toCTL(FRR, V, V1, CTL), write("\n =======CTL ========\n"), write(CTL),
		  write("\n =======CTL ========\n"),
		 retractall(pair(X,Y)), retractall(formula(X1,Y1,Z1)), retractall(prop(Z)), %在最后移除所有的pair和formula对，以便计算另外的公式
		 retractall(alpha(_,_,_)), retractall(beta(_,_,_)), retractall(snfclause(_, _, _, _)),
		write("\n Ins====="), write(R5), write(R6))
...
insAllL([], _, _, []). %该函数用于找到所有的实例化公式。（遍历）
...
formula_Ind([], _, [], []).
formula_Ind(_, [], [], []).
formula_Ind(L, Ind, LP, F) :- list_EX(L, Ind, LP),find_head(LP, HL), diff_head(LP, HL, HF), pro6_ii(HF, F). %将具有Ind的子句放在LP中，并求这些子句的化简，得到F。





list_EX([], _, []). %找到类型为ex具有相同index的EX子句的集合
list_EX(L, Ind, LP) :- findall(C, (C = snf_clause(ex, H, T, Ind1), member(C, L), Ind = Ind1), R), sort(R, LP).
/*
list_EX([], _, [], []). %找到具有相同index的EX子句的集合
list_EX([H|L1], Ind, LP) :- H = snf_clause(Type, Head, Tail, I1), %Ind是来源于将CTL公式转换为SNF子句集过程中引入的index的集合。
	(Ind = I1, H1 = H; H1 = []),
	list_EX(L1, Ind, LP1),
	(H1 = [], LP = LP1; append([H1], LP1, LP)).*/
	
find_head([], []). %找出相同index下的所有头部。
find_head(L1, L2) :- findall(Head, member(snf_clause(_, Head, _, _), L1), R), sort(R, L2).

diff_head(_, [], []).
diff_head([], _, []). %将具有相同头部的子句放在一起。
diff_head(L, [H|L1], L2) :- findall(T, member(snf_clause(ex, H, T, _), L), R), sort(R,R1), %assert(formula_ind(H, F)),
	diff_head(L, L1, L3),
	append([formula_ind(ex, H, R1)], L3, L2). %计算相同头部相同index的公式，即Proposition 6中的(i).	
/*
diff_head(L, [H|L1], L2) :- findall(T, member(snf_clause(_, H, T, _), L), R), sort(R,R1), cnf2wff(R1, F), %assert(formula_ind(H, F)),
	diff_head(L, L1, L3),
	append([formula_ind(H, F)], L3, L2). %计算相同头部相同index的公式，即Proposition 6中的(i).*/
