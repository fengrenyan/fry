...
Now assume a situation in which due to some problems (e.g. the venue changed or the chair broke down), John does no longer have a chair. This means, all the playing processes concerning ``$ch$" no more necessary and should be dropped from the  specifications (e.g. $\varphi$).
...
Similar scenarios  like the one presented in Example~\ref{examp:run} may arise in many different domains such as business-process modelling, planning, software development, concurrent systems and more~\cite{Baier:PMC:2008}. However, dropping some restrictions (atoms) in a large and complex  system or specification, without affecting the working system components or violating dependent specifications over a given signature, is a non-trivial task.
Moreover, in such a scenario, there are two highly informative logical notions introduced by E. Dijkstra in~\cite{DBLP:journals/cacm/Dijkstra75}:
% Moreover, in such a scenario, two logical notions introduced by E. Dijkstra in~\cite{DBLP:journals/cacm/Dijkstra75} are highly informative:
the strongest post-condition (SP) and the weakest precondition (WP) of a given specification, which are corresponding with the \emph{strongest necessary condition} (SNC) and the \emph{weakest sufficient condition}  (WSC), proposed by Lin~\cite{DBLP:Lin:AIJ:2001}, of the specification, respectively, and have been central to a wide variety of tasks and studies, e.g. in generating counterexamples~\cite{dailler2018instrumenting} and refinement of system~\cite{woodcock1990refinement}. These correspond to the \emph{most general consequence} and the \emph{most specific abduction} of such specification, respectively.
...
To address these scenarios and to target the relevant notions SNC (WSC) and knowledge update in a principle way. Inspired by~\cite{DBLP:Lin:AIJ:2001,renyansfirstpaper}, in this paper we explore the knowledge update and SNC (WSC) of $\mu$-calculus from the point of forgetting.
...
\emph{Forgetting},
which is a dual concept of {\em uniform interpolation}~\cite{visser1996uniform,konev2009forgetting} and was first formally defined in propositional and first-order logic (FOL) by Lin and Reiter~\cite{lin1994forget,eiter2019brief}, can be traced back to the work of Boole on propositional
variable elimination and the seminal work of Ackermann~\cite{ackermann1935untersuchungen}.
Moreover, it has been extended  to various logic systems, including modal logics~\cite{Yan:AIJ:2009,fang2019forgetting} and nonmonotonic logics~\cite{DBLP:journals/jair/WangZZZ14,gonccalves2020limits}. See~\cite{eiter2019brief} for a recent and comprehensive survey.
...
However, existing forgetting definitions in classical propositional logic (CPL) and answer set programming are not directly applicable in modal logics.
...
% The rest of the paper is organised as follows. Next section reports about the related work. Section~\ref{preliminaries} introduces the notation and technical preliminaries. As key contributions, Section~\ref{forgetting}, introduces the notion of forgetting in $\mu$-calculus. Moreover, it studies the semantic properties of forgetting. In addition, a complexity analysis, concerning model checking and reasoning problems of forgetting, is carried out.
% Section~\ref{ns_conditions} explores the relation between forgetting and SNC (WSC). Section~\ref{knowledge_updat} gives how to Represent knowledge update via forgetting. Conclusion closes the paper.
%
% Due to space restrictions, for most of the technical results, the actual proof is moved to the supplementary material in Appendix, and instead an intuitive justification is put in place.
After discuss the related work in the next section, the basic notation and technical preliminaries are introduced in Section~\ref{preliminaries}. The formal definition of forgetting in $\mu$-calculus,  its various properties and computational complexities are presented in Section~\ref{forgetting}.
Section~\ref{ns_conditions} identifies the weakest sufficient conditions via forgetting.
Section~\ref{knowledge_updat} addresses the knowledge update of $\mu$-calculus via forgetting.
Finally, concluding remarks are discussed in Section~\ref{sec:conclude}.
Due to space limitation, the detailed proofs of  technical results are moved to Appendix.

\section{Related work}
In this section, we shall discuss briefly about published matter that technically relates to our work.

\subsection{The Weakest precondition condition}
The Weakest precondition, as an important concept in formal verification, was proposed in~\cite{Dijkstra:1959} at first by Dijkstra to solve the problem of computing or approximating invariant appearing in \emph{verification of computer programs and systems}, particularly in ``Hoare triple"~\cite{Hoare1969}.
Afterwards, it has been wildly studied by researchers, especially in generating counterexamples~\cite{dailler2018instrumenting} and refinement of  system~\cite{woodcock1990refinement}.

In the field of knowledge representation and reasoning (KR), there is a similar concept, called \emph{weakest sufficient condition}(WSC), had been introduced in~\cite{DBLP:Lin:AIJ:2001} by Lin to generate successor state axioms from causal theories. Moreover, the SNC and WSC for a proposition $q$ on a restricted subset of the propositional variables under a propositional theory $T$ are computed based on the notion of forgetting.
Afterwards, the SNC and WSC are generalized to first order logic (FOL) and a direct method that is based
on \emph{Second-Order Quantifier Elimination} (SOQE) technique has been proposed to automatically generate SNC and WSC in~\cite{doherty2001computing}. Besides, in the paper~\cite{renyansfirstpaper}, the authors also use a forgetting-based method to compute the SNC and WSC in \CTL.

\subsection{The notion of forgetting}
\emph{Forgetting}, which was first formally defined in propositional and first order logics by Lin and Reiter~\cite{lin1994forget}, can be traced back to the work of Boole on propositional
variable elimination and the seminal work of Ackermann~\cite{ackermann1935untersuchungen} who recognised that the problem amounts to the elimination of existential second-order quantifiers.
Usually, the definition of forgetting can be defined from the perspective of Strong/Semantic Forgetting and Weak Forgetting  respectively~\cite{zhang2010forgetting}.

In FOL, forgetting has often been studied as an instance of the SOQE problem. It shows in~\cite{lin1994forget} that the result of (Strongly) forgetting an n-ary predicate $P$ from a first-order formula $\varphi$ is $\exists R \varphi[P/R]$, in which $R$ is an n-ary predicate variable and $\varphi[X/Y]$ is a result of replacing every occurrence of $X$ in $\varphi$ by $Y$. The task of forgetting in first-order logic, as a computational problem, is to find a first-order formula that is equivalent to $\exists R \varphi[P/R]$. It is evident that this is a SOQE problem. However, it shows in~\cite{gabbay2008second} that the solution of SOQE problem is not always expressible in FOL, which means that the results of forgetting in FOL is not always expressible in FOL.
...
FOL formulas~\cite{zhang2010forgetting}.
Similarly, the forgetting in description logics are also explored to create restricted views
of ontologies by eliminating concept and role symbols from description logic-based
ontologies~\cite{Wang:AMAI:2010,Lutz:IJCAI:2011,zhao2017role}.

In CPL, forgetting has often been studied under the name of variable
elimination. In particular, the solution of forgetting a propositional variable $p$ from a CPL formula $\varphi$ is $\varphi[p/\bot] \vee \varphi[p/\top]$~\cite{lin1994forget}, where $\varphi[p/\bot]$ and $\varphi[p/\top]$ denote the formulas obtained from $\varphi$ by replacing atom $p$ with $\bot$ and $\top$ respectively.
In~\cite{Yan:AIJ:2009}, the authors define the knowledge forgetting of S5 modal logic from the Strong Forgetting point of view to explore the relationship between knowledge forgetting and knowledge update. Besides, they have proposed four general postulates for knowledge forgetting and show that these four postulates precisely characterize the notion of knowledge forgetting described above in S5 modal logic.
What's more, forgetting in logic programs under answer-set semantics, which is a non-monotonic logic program, are also considered, see~\cite{DBLP:Zhang:AIJ2006,Eiter2008Semantic,Wong:PhD:Thesis,DBLP:journals/jair/WangZZZ14,Yisong:IJCAI:2013,gonccalves2020limits}  for more details.


\section{Modal $\mu$-calculus} \label{preliminaries}
We start with some technical and notational preliminaries.
Let $V\subseteq \Ha$ be a set of atoms, then the set $\Ha -V$ is denoted as $\overline{V}$.
...
Deciding whether a disjunctive formula is satisfiable can be done in polynomial time, hence changing the $\mu$-calculus formula into its disjunctive form will increase the efficiency of deciding its satisfiability since the satisfiability of $\mu$-calculus is  $\textsc{Exptime}$-complete.
...
And last, we show that the model checking problem of forgetting $V$ from a disjunctive formula is in \textsc{NP} $\cap$ co-\textsc{NP} and the reasoning problems are $\textsc{Exptime}$-complete.
...
That is, the models of such specification will be extended to some other Kripke structures such that those Kripke structures simulate the existing models on the remaining signature and  vice versa.
...
In this sense, we can see that our $V$-bisimulation is also closed to those definitions to some extent.
...
In this case, we also say that $\Hm_1$ and $\Hm_2$ are bisimilar on $V$.
...
(ii) refers to that if $\Hm_1$ and $\Hm_2$ are bisimilar on two different sets $V$ and $V_1$ respectively, then they are bisimilar on the union of these two sets.
...
update of $\mu$-calculus. Moreover, as shown in~\cite{renyansfirstpaper}, forgetting in \CTL\ can also be used to compute the SNC (WSC) and we shall report this in $\mu$-calculus.
...
At this point, it is important to emphasize that, the notion of forgetting  we have defined for $\mu$-calculus extends the classical forgetting defined for CPL~\cite{lin1994forget}. Assuming $\varphi$ is a CPL formula and $p\in \Ha$, then $\Forget(\varphi, p)$ is a result of forgetting $p$ from $\varphi$; that is, $\Forget(\varphi, p)\equiv \varphi[p/\bot] \vee \varphi[p/\top]$.
Moreover, given a set $V\subseteq \Ha$, the $\Forget(\varphi, V\cup \{p\})$ is recursively defined as $\Forget(\Forget(\varphi, p),V)$ with $\Forget(\varphi, \emptyset) = \varphi$.
% one can recursively define $\Forget(\varphi, V\cup \{p\}) = \Forget(\Forget(\varphi, p),V)$,
% where $\Forget(\varphi, \emptyset) = \varphi$.
Using this insight, the following result shows that the classical notion of forgetting (for CPL ~\cite{lin1994forget}) is a special case of forgetting in $\mu$-calculus.
...
(1) The step of computing forgetting by $\forget$ is as follows:
\begin{align*}
...
It is obvious that $\forget(\alpha, V) \equiv \Muforget(\alpha, V)$.
\end{example}

%The closure of $\Muforget$ shows the result of forgetting always in $\mu$-calculus, however,
...
say that  forgetting results have no effect on formulas that are irrelevant to atoms to be forgotten; the postulate (\textbf{\IR}) states that
...
The following theorem says that the forgetting postulates above indeed precisely characterize the underling forgetting semantics of $\mu$-calculus.
...
Postulate $\IR$ is also of crucial importance for computing SNC and WSC, besides the representation theorem. Consider the $\mu$-sentence $\psi = \varphi \wedge (q \lrto \alpha)$. If $\varphi \wedge \alpha$ is $\{q\}$-irrelevant, then the result of forgetting $q$ from $\psi$ is $\varphi$. Formally, it can be described in the following lemma, and as we will  see later in Section 4, it is the base of reducing the SNC (WSC) of any $\mu$-sentence to that of a proposition.
...
Another important property of $\Muforget$ is about the formulas with that all the sub-formula are in the domain of $\ALL\NEXT$ ($\square$) or $\EXIST \NEXT$ ($\lozenge$). Formally:
...
Although the homogeneity in Proposition~\ref{pro:mu:forget:2} gives a convenience for computing forgetting.
However, it is noteworthy that it is easy to compute the result of forgetting a set of atoms from a $\mu$-sentence when it is a disjunctive formula. By Theorem~\ref{thm:exist} we know that the set of models of the result of forgetting a proposition $p$ from a $\mu$-sentence $\varphi$ is equal to the set of models of the uniform interpolant  of $\varphi$ with respect to $\Var(\varphi) -\{p\}$, i.e. $\Mod(\Muforget(\varphi, \{p\})) = \Mod(\widetilde{\exists}p \varphi)$.  This means that when the given formula is a disjunctive formula, we can compute the result of forgetting $p$ from $\varphi$ just by replacing the $p$ and $\neg p$ with $\top$ at the same time. Then we have the following proposition.
 \begin{proposition}
 Let $\varphi$ be a $\mu$-sentence and $p\in \Ha$. If $\varphi$ is a disjunctive formula~\cite{d2006modal}, then $\Muforget(\varphi, \{p\})$ can be computed in linear time.
 \end{proposition}
 \begin{proof}
...
In this sense, we can transform a formula into its disjunctive form offline, and then compute its result of forgetting some atoms from it, this will be efficient in some situations.
 Let's recall the three disjunctive formulas, which obtained from  $\mu$-sentences, in Example~\ref{exmp:disF}. And in the following example, we shall show how to compute forgetting $ch$ from those formulas.

 \begin{example}
 Let's denote those formulas as follows:
...
In this section, we will explore the complexity of the forgetting operator from both its model checking and reasoning problems.
Recall that the result of forgetting some atoms from a disjunctive formula can be obtained in linear time, nevertheless, as we will show that the model checking problem is intractable even the given formula is a disjunctive formula.

Before talk about the complexity results of the forgetting operator, let's recall the $\mu$-automaton, which is an important technique in $\mu$-calculus to compute the uniform interpolation of a given formula and demonstrate the complexity of model checking of $\mu$-calculus. In this part we will show that the $\mu$-automaton can also make a big convenience to show our complexity results.
...
\end{definition}
 ...
 Although this automaton differs slightly from those given in~\cite{janin1995automata,bradfield2018mu}, but the automata in their various guises are equivalent. Moreover, modal automata are essentially alternating automata~\cite{bradfield2018mu}. And it has been shown that constructing a $\mu$-automaton from a $\mu$-calculus formula can be done in exponential time, while it is in polynomial time when the $\mu$-calculus formula is a disjunctive formula.

 In addition, it had been shown in~\cite{d1996uniform} that for each $\mu$-automaton $A_{\varphi}$ obtained from a $\mu$-formula $\varphi$  and a proposition $p$ it is easy to construct another $\mu$-automaton $B$ such that, for any Kripke structure ${\cal N}$, $B$ accepts ${\cal N}$ iff there is a model ${\cal N}'$ of $\varphi$ such that ${\cal N} \lrto_{\{p\}} {\cal N}'$, i.e. $B$ corresponds to a $\mu$-sentence which is equivalent to $\Muforget(\varphi, V)$ by the definition of forgetting in $\mu$-calculus.
 Then we have the following complexity result.

\begin{proposition}[Model Checking]\label{pro:MC}
Given a finite Kripke structure  $\Hm$, a disjunctive formula $\varphi$ and $V\subseteq \Ha$, deciding $\Hm \models^? \Muforget(\varphi, V)$ is in \textsc{NP}$\cap$co-\textsc{NP}.
\end{proposition}
% \begin{proof}
...
\end{itemize}
\end{definition}
Intuitively, the SNC (WSC) is the strongest (weakest) one among the NCs (SCs) of $q$ on $V$ under $\phi$.
Note that if both $\psi$ and $\psi'$ are SNC (WSC) of $q$ on $V$ under $\phi$, then
$\psi\equiv \psi'$, i.e., they are the  logical consequence of each other.
...
Replacing $q$ by any $\mu$-sentence $\alpha$ and redefining $V$  as a subset of $\Var(\alpha) \cup \Var(\phi)$ in Definition~\ref{def:NC:SC}, then this definition is generalised to arbitrary formulas.
...
The following result shows that the forgetting and the notion of SNC (WSC) are closely related, which are central to our contribution.
...
Following Theorem~\ref{thm:SNC:WSC:forget}, assume that $\psi = \Muforget(\varphi \wedge q, (\Var(\varphi) \cup \{q\})- V)$.  Then, $\varphi \wedge q \models \psi$  by (\W). Moreover,  $\varphi \wedge q \models \psi$,  and then $\psi$ is a NC of $q$ on $V$ under $\varphi$.

In addition, for any $\mu$-sentence $\psi'$ with $\IR(\psi', (\Var(\varphi) \cup \{q\})- V)$ and $\varphi \wedge q \models \psi'$,
we have $\psi \models \psi'$ by (\PP). Therefore, $\psi$ is the SNC of $q$ on $V$ under $\varphi$. This shows the intuition of how the SNC can be obtained from the forgetting.

Although we can use forgetting to compute the SNC and WSC, it had been found that sometimes, it is a lot easier
to compute the strongest necessary condition than the weakest sufficient condition, and
sometimes, it is the other way around~\cite{DBLP:Lin:AIJ:2001}. For instance, the author had found that for many
theories, strongest necessary conditions are easier to compute than weakest sufficient
conditions. When one condition is much easier to compute, the following proposition will
be very helpful in computing the other one.

\begin{proposition}\label{pro:eachOne}
Let $\Gamma$ be a $\mu$-sentence, $q$ a proposition, and $V$ a set of propositions.
\begin{enumerate}[(i)]
...
\section{Representing knowledge update via forgetting}\label{knowledge_updat}
...
Furthermore, we have studied formal properties about forgetting, including homogeneity, modularity and commutativity.
In particular, we have shown that our notion of forgetting satisfies the well-known forgetting postulates. Thus, it faithfully extends the notion of forgetting in classical propositional logic, modal logic \SFive\  and \CTL.
...
we have investigated that the model checking problem for forgetting results  from a disjunctive formula is in \textsc{NP}$\cap$co-\textsc{NP}
and the entailment problems relating to forgetting are $\textsc{Exptime}$-complete. % from the point of $\mu$-automaton.
...
In the future, an algorithm to compute forgetting will be explored and implemented. As it is shown in this work that the forgetting of a disjunctive formula can be computed in linear time, in this sense, a method of transforming a formula into its disjunctive form will be a direction for the forgetting algorithm.
...
Besides, It is worthwhile to explore sub-classes of $\mu$-calculus that the forgetting can be computed easily.
Moreover, when a finite transition system $\Hm$ does not satisfy a specification $\varphi$,
it is important to calculate a weakest sufficient condition $\psi$ over a signature $V$ under which $\Hm$ satisfies $\varphi$. %viz. expressing $\Hm$ with its characterizing formula, to explore how the condition $\psi$ can guide the design of a new transition system $\Hm'$ satisfying $\varphi$.
...
\begin{proof}
Let $A_{\varphi}$ be a $\mu$-automaton such that, for any Kripke structure ${\cal N}$, %there is
$A_{\varphi}$ accepts ${\cal N}$ iff ${\cal N} \models \varphi$, where $A_{\varphi} = (Q, \Sigma_p, \Sigma_r, q_0, \delta, \Omega)$ with $\Var(\varphi) = \Sigma_p \cup \Sigma_r$. Without loss of generality, we assume $V \subseteq \Var(\varphi)$ and $V=\{p\}$.  Therefore we can construct  a $\mu$-automaton $B= (Q, \Sigma_p - V, \Sigma_r, q_0, \delta', \Omega)$ such that
\[
    \delta'(q, L) = \delta(q, L) \cup \delta(q, L \cup \{p\}).
\]
...
\begin{enumerate}[(i)]
 ...
 \end{enumerate}
\begin{proof}
(i) First of all, $\varphi \wedge \psi$ is an SC due to $\Gamma \cup \{\varphi\} \models \psi \supset q$, i.e. $\Gamma \models \varphi \wedge \supset q$.
We will show it is the weakest one: suppose $\varphi'$ is an SC, i.e. $\Gamma \models \varphi' \supset q$. We need to show that $\Gamma \models \varphi' \supset \varphi \wedge \psi$. Thanks to $\Gamma \models q \supset \varphi$. We have $\Gamma \models  \varphi' \supset \varphi$. However, $\varphi'$ is also an SC of $q$ under $\Gamma \cup \{\varphi\}$, hence $\Gamma \cup \{\varphi\} \models  \varphi' \supset \psi$ because $\psi$ is the WSC of $q$ under $\Gamma \cup \{\varphi\}$. Therefore $\Gamma \models (\varphi \wedge \psi)$.

(ii) Suppose $\varphi'$ is an NC of $q$ under $\Gamma$, i.e. $\Gamma \models q \supset \varphi'$. We have $\Gamma \models \psi \supset \varphi'$ since $\Gamma \models \psi \supset q$. And there is $\Gamma \cup \{\neg \psi\} \models \varphi \supset \varphi'$ due to $\Gamma \cup \{\neg \psi\} \models q \supset \varphi$ and $\Gamma \cup \{\neg \psi\} \models q \supset \varphi'$. Therefore we have $\Gamma \models \varphi \supset \varphi'$ by $\Gamma \models \neg \psi \wedge \varphi \supset \varphi'$ and $\Gamma \models \psi \supset \varphi'$.
\end{proof}
