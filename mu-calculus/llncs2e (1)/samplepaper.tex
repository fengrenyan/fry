...
Another important property of $\Muforget$ operator is about the formulas with that all the sub-formula are in the domain of $\ALL\NEXT$ ($\square$) or $\EXIST \NEXT$ ($\lozenge$). Formally:
...
However, it is noteworthy that it is easy to compute a forgetting results for a set of atoms from a $\mu$-sentence when it is a disjunctive formula. By Theorem~\ref{thm:exist} we know that the set of models of the result of forgetting a proposition $p$ from a $\mu$-sentence $\varphi$ is equal to the set of models of the uniform interpolant  of $\varphi$ with respect to $\Var(\varphi) -\{p\}$, i.e. $\Mod(\Muforget(\varphi, \{p\})) = \Mod(\widetilde{\exists}p \varphi)$.  This means that when the given formula is a disjunctive formula, we can compute the result of forgetting $p$ from just by replacing the $p$ and $\neg p$ with $\top$ at the same time. Then we have the following proposition.
 \begin{proposition}
...
Hence, classify the forgetting operator from its complexity is important to explore an efficient algorithm to compute it.
