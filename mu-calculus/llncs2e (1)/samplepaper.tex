...
To address these scenarios and to target the relevant notions SNC (WSC) and knowledge update in a pricipled way. Inspired by~\cite{lin2001strongest,feng2020sufficient}, in this paper we explore the knowledge update and SNC (WSC) of $\mu$-calculus from the point of forgetting.
In particular, we will give the definition of forgetting in $\mu$-calculus by using the bisimulation~\cite{browne1988characterizing,Baier:PMC:2008,Yan:AIJ:2009} and show whether this notion satisfies the general principles or postulates proposed by Zhang~\cite{Zhang2009Knowledge}. We then study the relationship between SNC (WSC) and forgetting, and we also demonstrate how forgetting can be used in knowledge update in $\mu$-calculus.

\emph{Forgetting},
which is a dual concept of {\em uniform interpolant}~\cite{visser1996uniform,konev2009forgetting} and was first formally defined in propositional and FOL by Lin and Reiter~\cite{lin1994forget,eiter2019brief}, can be traced back to the work of Boole on propositional
...
Informally, the four postulates proposed by Zhang~\cite{Zhang2009Knowledge} show that the result $\psi$ of forgetting some set $V$ of atoms from a formula $\varphi$ is not only weaker than the $\varphi$, i.e. $\varphi \models \psi$, irrelevant to $V$, i.e. exists some formula that do not contain atoms in $V$ and equivalent with $\psi$, and also has the same ``logic content" with $\varphi$, i.e. for each formula $\phi$ that irrelevant to $V$, $\phi$ can be implied by $\varphi$ iff $\phi$ can be implied by $\psi$.
...
\begin{itemize}
    ...
    \item $r\in S$,
    \item $R$ is a binary relation on $S$, i.e. $R \subseteq S \times S$, called transition relation, and
    \item $L: S \rto 2^{\Ha}$ is a labeling function.
\end{itemize}
A Kripke structure $\Hm$ is finite if $S$ is finite and $L(q) = \emptyset$ for almost all $q\in \Ha$.

Given a Kripke structure $\Hm$ and a valuation $V: {\cal V} \rto 2^S$, the set of states
in which a formula $\varphi$ is true, denoted $\parallel \varphi\parallel_V^{\Hm}$, is defined inductively as follows (we will omit superscript $\Hm$ when it causes no ambiguity):
\begin{align*}
    & \parallel p\parallel_V = \{s\ |\ p \in L(s)\} \ \ \parallel\top\parallel_V = S \ \ \parallel\bot\parallel_V = \emptyset \\
    & \parallel\neg p\parallel_V = S- \parallel p\parallel_V\\
    & \parallel X\parallel_V = V(X)\\
    & \parallel\varphi_1 \vee \varphi_2\parallel_V = \parallel\varphi_1\parallel_V \cup \parallel\varphi_2\parallel_V\\
    & \parallel\varphi_1 \wedge \varphi_2\parallel_V = \parallel\varphi_1\parallel_V \cap \parallel\varphi_2\parallel_V\\
    & \parallel\lozenge \varphi\parallel_V = \{s| \exists s'. (s, s') \in R \wedge s' \in \parallel\varphi\parallel_V\}\\
    & \parallel\square \varphi\parallel_V = \{s| \forall s'. (s, s') \in R \Rto s' \in \parallel\varphi\parallel_V\}\\
    & \parallel \mu X. \varphi\parallel_V = \bigcap\{S' \subseteq S | \parallel\varphi\parallel_{V[X:= S']} \subseteq S'\}\\
    & \parallel \nu X. \varphi\parallel_V = \bigcup\{S' \subseteq S | S' \subseteq \parallel\varphi\parallel_{V[X:= S']}  \}
\end{align*}
here $V[X:= S']$ is equal to the valuation function $V$ except that $S'$ is assigned to $X$. Note that $\parallel \mu X. \varphi\parallel_V$ is the least fixpoint of the $\varphi(X)$.

In the following, we denote $s\in \parallel \varphi \parallel_V$ by $(\Hm, s, V) \models \varphi$ and we may leave out the
...
By the Tarski-Knaster theorem, least and greatest fixpoints of monotonic functions $f$ over subsets of a set $U$ can be defined by transfinite induction, i.e., the least fixpoint $\mu(f) = \bigcup\mu_{\alpha}(f)$, where
\begin{itemize}
...
Therefore, $\parallel \mu X. \varphi(X)\parallel_V$ is the least fixpoint of the monotone operator $\parallel \varphi \parallel_{V}: 2^S\rto 2^S$ (it also be written as $Y \mapsto \parallel \varphi\parallel_{V[X:=Y]}$ sometimes).
...
\begin{itemize}
    \item $\nu_0(\parallel \varphi \parallel) = S$,
    \item $\nu_1(\parallel \varphi \parallel) = \parallel j \parallel \cap \parallel ch \parallel \cap \parallel \square (\neg j \wedge \neg ch) \parallel \cap \parallel \square \square \nu_0(\parallel \varphi \parallel) \parallel$ $=\{s | s \in L(j) \cap L(ch)\ \text{and}\ \forall t. (s, t) \in R, t \in S - (L(j) \cup L(ch))\}$,
    \item ...
\end{itemize}
(2) For $\varphi=\nu X. (j \wedge ch) \wedge \lozenge (\neg j \wedge \neg ch) \wedge \lozenge \lozenge X$, let $\Hm=(S, r, R, L)$ be a Kripke structure, then we have:
\begin{itemize}
    \item $\nu_0(\parallel \varphi \parallel) = S$,
    \item $\nu_1(\parallel \varphi \parallel) = \parallel j \parallel \cap \parallel ch \parallel \cap \parallel \square (\neg j \wedge \neg ch) \parallel \cap \parallel \square \square \nu_0(\parallel \varphi \parallel) \parallel$ $=\{s | s \in L(j) \cap L(ch)\ \text{and}\ \exists t. (s, t) \in R, t \in S - (L(j) \cup L(ch))\}$,
    \item $\nu_2(\parallel \varphi \parallel) = \parallel j \parallel \cap \parallel ch \parallel \cap \parallel \square (\neg j \wedge \neg ch) \parallel \cap \parallel \square \square \nu_1(\parallel \varphi \parallel) \parallel$
    \item ...
\end{itemize}
For the Kripke structure $\Hm$ in (a) of Example~\ref{examp:run}, we have $\parallel \varphi \parallel = \{s_0\}$.
\end{example}

An alternative syntax for the $\mu$-calculus is obtained by substituting the $\lozenge$ operator with
a set of \emph{cover\ operators}, one for each natural n. For $n \geq 1$ these operators are defined as
...
in a Kripke structure $\Hm$ as follows: $Cover(\emptyset)$ is true in $\Hm$ if and only if the root of $\Hm$ does not have any successor, while $Cover(\varphi_1, \dots, \varphi_n)$ is true in $\Hm$ if and only if the successors
of the root are covered by $\varphi_1, \dots, \varphi_n$. More formally, $(\Hm, s,V ) \models Cover(\varphi_1, \dots, \varphi_n)$
if and only if:
\begin{itemize}
    \item for every $i = 1, . . . , n$ there exists $t$ with $(s, t) \in \Hm$ and $(\Hm, t,V) \models \varphi_i$;
    \item for every $t$ with $(s, t) \in \Hm$ there exists $i\in \{1, . . . , n\}$ with $(\Hm, t,V) \models \varphi_i$.
\end{itemize}
...
as we shall see, cover operators behave nicely with respect to the definition of disjunctive formula. Specially, the $\mu$-calculus formula have a normal form, called disjunctive formula~\cite{janin1995automata} as follows.
...
Another important conept is \emph{uniform interpolant}, which has been widely explored in various of logic languages. Some logics enjoy uniform interpolation, but some are not.
Formally, the uniform interpolant under $\mu$-calculus is defined as follows.
\begin{definition} [Uniform interplant~\cite{d2006modal}]
  Given a $\mu$-sentence $\varphi$ and a language $L'\subseteq L(\varphi)$, the uniform interpolant of $\varphi$ with respect to $L'$
 is a $\mu$-sentence $\psi$ such that:
 \begin{itemize}
     \item $\varphi \models \psi$;
     \item whenever $\varphi \models \varphi_1$ and $L(\varphi) \cap L(\varphi_1) \subseteq L'$ then $\psi \models \varphi_1$;
     \item $L(\psi) \subseteq L'$.
 \end{itemize}
\end{definition}

Notice that uniform interpolation is stronger than Craig interpolation, which states that for any two formulas $\varphi$, $\psi$ with $\varphi \models \psi$ there exists a formula $\theta$ (called the Craig interpolant of $\varphi$, $\psi$) in the common language with $\varphi \models \theta$ and $\theta \models \psi$. Clearly, if the logic enjoys uniform interpolation then the Craig interpolant of $\varphi$, $\psi$ does not depend on $\psi$ but only on the common language: it is simply the uniform interpolant of $\varphi$ relative to $L' = L(\varphi) \cap L(\psi)$.
...
First, we give the definition of $V$-bisimulation between Kripke structures. The notion of $V$-bisimulation capture the idea that the two systems are behaviourally same except the atoms in $V$. In this way we give the definition of forgetting by using the $V$-bisimulation.

Second, the related properties, e.g. Modularity, Commutativity and Homogeneity, of the forgetting operator will be explored.
And last, we show that the Model checking problem of forgetting $V$ from a disjunctive formula is in \textsc{NP} $\cap$ co-\textsc{NP} and the reasoning problems are in  $\textsc{Exptime}$-complete.
...
For convenience, in the following we let $\Hm_i=(S_i, r_i, R_i, L_i)$ with $i\in \mathbf{N}$ be Kripke structure.
...
\begin{definition}[V-bisimulation]\label{def:VB}
  ...
  \begin{itemize}
      ...
      \item $(s, s')\in R_1$ and $s \Hb t$ implies that there is a $t'$ such that $s' \Hb t'$ and $(t, t')\in R_2$, and
      \item vice versa: if $s \Hb t$ and $(t, t')\in R_2$ then there is an $s'$ with $(s, s')\in R_1$ and $t' \Hb s'$.
  \end{itemize}
\end{definition}
Two Kripke structures $\Hm_1$ and $\Hm_2$ are \emph{$V$-bisimilar}, denoted $\Hm_1 \lrto_V \Hm_2$, if there exists a $V$-bisimulation ${\cal B}$ between them.
The $V$-bisimulation is similar with the ${\cal L}$-bisimulation in~\cite{d1996uniform}, which is a relation satisfying the above clauses just for the symbols in language ${\cal L}$.
From this definition we know that the $\lrto_V$ is an equivalence relation between Kripke structures. Formally:
...
Supposing $\Hm_1 \lrto_V \Hm_2$ by the $V$-bisimulation $\Hb_1$ and $\Hm_2 \lrto_{V_1} \Hm_3$  by the $V_1$-bisimulation $\Hb_2$. Let $\Hb = \{(s_1, s_3)| (s_1, s_2) \in \Hb_1\ and\ (s_2, s_3) \in \Hb_2\}$. We can easily check that $\Hb$ is a $(V\cup V_1)$-bisimulation between $\Hm_1$ and $\Hm_3$.
...
It should then be obvious that if $\IR(\varphi, V)$ and
$\Hm \lrto_V \Hm'$ then $\varphi$ holds in $\Hm$ iff it holds in $\Hm'$.
\textbf{Therefore, in the following we only consider the $\mu$-sentence.}
...
update of $\mu$-calculus. Moreover, as shown in~\cite{renyansfirstpaper}, forgetting in \CTL\ can also be used to compute the SNC and the WSC.
...
The $V$-{\em irrelevant} of a set of formulas can be defined similarly, i.e. a set $\Sigma$ of formulas is irrelevant to the atoms in $V$, written $\IR(\Sigma, V)$, if $\IR(\varphi, V)$ for each $\varphi \in \Sigma$.
...
At this point, it is important to emphasize that, the notion of forgetting  we have defined for $\mu$-calculus respects the classical forgetting defined for propositional logic (PL)~\cite{lin1994forget}. Assuming $\varphi$ is a PL formula and $p\in \Ha$, then $\Forget(\varphi, p)$ is a result of forgetting $p$ from $\varphi$; that is, $\Forget(\varphi, p)\equiv \varphi[p/\bot] \vee \varphi[p/\top]$.
...
The other parts can be similarly proved as Theorem 4 in~\cite{renyansfirstpaper}.
\end{proof}
...
The homogeneity of $\ALL\NEXT$ (or $\EXIST\NEXT$) on forgetting says we can move the operator $\Muforget$ afterward to the $\ALL\NEXT$ (or $\EXIST\NEXT$) to compute the forgetting of formula of the form $\ALL\NEXT \varphi$ (or $\EXIST\NEXT \varphi$).
...
%   It is easy to check that $\Muforget(\delta X. \phi(X), V) \not \equiv \delta X. \Muforget(\phi(X), V)$ with $\delta \in \{\mu, \nu\}$ since $\phi(X)$ is not a $\mu$-sentence.
 ...
 It is noteworthy that it is easy to compute forgetting a set of atoms from a $\mu$-sentence when it is a disjunctive formula.
 \begin{proposition}
...
\end{proposition}
\begin{proof}
Let $A_{\varphi}$ be a $\mu$-automaton such that for any Kripke structure ${\cal N}$ there is $A_{\varphi}$ accept ${\cal N}$ iff ${\cal N} \models \varphi$. Where $A_{\varphi} = (Q, \Sigma_p, \Sigma_r, q_0, \delta, \Omega)$ with $\Var(\varphi) = \Sigma_p \cup \Sigma_r$. Without loss of generality, we assume $V \subseteq \Var(\varphi)$ and $V=\{p\}$.  Therefore we can constructure a $\mu$-automaton $B= (Q, \Sigma_p - V, \Sigma_r, q_0, \delta', \Omega)$ with:
\[
    \delta'(q, L) := \delta(q, L) \cup \delta(q, L \cup \{p\}).
\]

It has been proved in~\cite{d1996uniform} that for each Kripke structure ${\cal N}$,  $B$ accept ${\cal N}$ iff there is a model ${\cal N}'$ of $\varphi$ such that ${\cal N} \lrto_{\{p\}} {\cal N}'$, i.e. $B$ corresponds to a $\mu$-sentence which is equivalent to $\Muforget(\varphi, V)$ by the definition of forgetting in $\mu$-calculus.

In this case, the problem $\Hm \models^? \Muforget(\varphi, V)$ is reduced to decide whether $B$ accept $\Hm$, which is \textsc{NP} $\cap$ co-\textsc{NP}~\cite{bradfield2018mu}.
\end{proof}
...
, we can obtain the complement $C$ of $A_\psi$ in linear time, and then the intersection $A_{C \cap B}$ between $C$ and $B$  in linear time. In this case, the $\Muforget(\varphi, V ) \models^? \psi$ is reduced to decide whether the language accpeted by $A_{C \cap B}$ is empty, which is $\textsc{Exptime}$-complete~\cite{comon1997tree}.
\end{proof}
...
\[
    \Hm \models \psi \mbox{ iff there is a model } \Hm' \mbox{ with } \Hm \lrto_V \Hm' \mbox{ and } \Hm' \models \phi.
\]
...
Theorem~\ref{thm:minU} means that our definition of knowledge update in $\mu$-calculus by forgetting is accordance with that by the $\leq_{\Hm}$, which is similar with the partial order $\leq_{I, pam}$ in CPL.
...
% \section{The relation of forgetting between \CTL\ and $\mu$-calculus}
% In this part, for convenience, we use $\ALL\NEXT$ for $\square$ and $\EXIST\NEXT$ for $\lozenge$.
% The definition of the alternation depth of $ad(\varphi)$ of formula $\varphi$ as follows.
% \begin{itemize}
%     \item $ad(p)=ad(X)=0$ for propositions $p$ and variables $X$;
%     \item $ad(\varphi \wedge \psi)= ad(\varphi \vee \psi) = max\{ad(\varphi), ad(\psi)\}$;
%     \item $ad(\neg \varphi) = ad(\varphi)$;
%     \item $ad(\mu X. \varphi) = 1 + max\{ad(\nu Y. \psi)\ |\ \nu Y. \psi$ is a subformula of $\varphi$ in which $X$ occurs free$\}$;
%     \item $ad(\nu X. \varphi) = 1 + max\{ad(\mu Y. \psi)\ |\ \mu Y. \psi$ is a subformula of $\varphi$ in which $X$ occurs free$\}$.
% \end{itemize}

% Let $L\mu_k$ denote the set of $\mu$-formulas restricted to formulas of alteration depth at most $k$. ($L\mu_1 \subseteq N_2$, where $N_2$ is defined in~\cite{d2006modal})

% The fixpoint characterization of \CTL\ basic modalities in terms of least or greatest fixpoints as follows (note that each is a formula of $L\mu_1$ and $Z\in {\cal V}$):
% \begin{align*}
%     & \EXIST\FUTURE \varphi \equiv \mu Z \cdot \varphi \vee \EXIST\NEXT Z \\
%     & \ALL\GLOBAL \varphi \equiv \nu Z \cdot \varphi \wedge \ALL \NEXT Z\\
%     & \ALL\FUTURE \varphi \equiv \mu Z \cdot \varphi \vee \ALL \NEXT Z\\
%     & \EXIST\GLOBAL \varphi \equiv \nu Z \cdot \varphi \wedge \EXIST\NEXT Z\\
%     & \ALL(\varphi \UNTIL \psi) \equiv \mu Z \cdot \psi \vee(\varphi \wedge \ALL \NEXT Z)\\
%     & \EXIST(\varphi \UNTIL \psi) \equiv \mu Z \cdot \psi \vee(\varphi \wedge \EXIST\NEXT Z)\\
%     & \EXIST(\varphi \UNLESS \psi) \equiv \nu Z \cdot \psi \wedge (\varphi \vee \EXIST\NEXT Z)\\
%     & \ALL(\varphi \UNLESS \psi) \equiv \nu Z \cdot \psi \wedge (\varphi \vee \ALL \NEXT Z).
% \end{align*}

% These fixpoint characterizations of \CTL\ basic modalities provide the key for translating all of \CTL\ into the $\mu$-calculus, viz., $L \mu_{1}$~\cite{emerson1980characterizing,emerson1986efficient}.
%  In this part we assume that the transition relation of the Kripke structure $\Hm$ is a \textbf{total relation}, i.e. for each state $s\in S$ there is a $s'\in S$ such that $(s,s') \in R$, to show the relation between the forgetting in \CTL\ and that in $\mu$-calculus.

% \begin{lemma} \label{lem:addFT}
% Let $X\in {\cal V}$ and $\varphi$ be a formula, then we have:
% \begin{enumerate}[(i)]
%     \item $\mu X. \ALL\NEXT X \equiv \mu X.(\bot \vee \ALL \NEXT X) \equiv \ALL \FUTURE \bot \equiv  \bot$;
%     \item $\nu X. \ALL\NEXT X \equiv \nu X.(\top \wedge \ALL \NEXT X) \equiv \ALL \GLOBAL \top \equiv  \top$;
%     \item $\mu X. \varphi \wedge \ALL\NEXT X \equiv \mu X.(\bot \vee (\varphi \wedge \ALL \NEXT X)) \equiv  \bot$;
%     \item $\nu X. \varphi\vee \ALL\NEXT X \equiv \nu X.(\top \wedge (\varphi \vee \ALL \NEXT) X)  \equiv  \top$.
% \end{enumerate}
% These are also correct to the $\EXIST$ path quantifier.
% \end{lemma}
% \begin{proof}
% We prove the $\ALL$ path quantifier case, the $\EXIST$ path quantifier case can be proved similarly.

% (i) We will prove this results by proving $\parallel \mu X. \ALL\NEXT X \parallel_V^{\Hm} = \parallel \mu X.(\bot \vee \ALL \NEXT X) \parallel_V^{\Hm}$ for all $\Hm$ and $V$. Therefore we omit the superscript $\Hm$ and subscript $V$.

% (a) $\mu_0(\parallel \ALL\NEXT X \parallel) = \emptyset$, then \\
% $\mu_1(\parallel \ALL\NEXT X \parallel) =\parallel \ALL\NEXT(\mu_0(\parallel \ALL\NEXT X \parallel)) \parallel = \parallel \ALL\NEXT(\emptyset) \parallel = \{s |s$ has no successor state$\} = \emptyset$ since the $R$ in our $\Hm$ is a \textbf{total relation}, i.e. for each state $s\in S$ there is a $s'\in S$ such that $(s,s') \in R$.

% (b) $\mu_0(\parallel \bot \vee \ALL \NEXT X \parallel) = \emptyset$, then\\
% $\mu_1(\parallel \bot \vee \ALL \NEXT X \parallel) = \parallel \bot \parallel \cup \parallel \ALL\NEXT(\mu_0(\parallel \ALL\NEXT X \parallel)) \parallel = \emptyset$, the reason is the same as (a).

% It is evident that $\Mod(\mu X. \ALL\NEXT X) = \Mod( \mu X.(\bot \vee \ALL \NEXT X)) = \Mod(\ALL \FUTURE \bot)= \emptyset$.


% (ii) We will prove this results by proving $\parallel \nu X. \ALL\NEXT X \parallel_V^{\Hm} = \parallel \nu X.(\top \wedge \ALL \NEXT X) \parallel_V^{\Hm}$ for all $\Hm$ and $V$. Therefore we also omit the superscript $\Hm$ and subscript $V$.

% (a) $\nu_0(\parallel \ALL\NEXT X \parallel) = S$, then\\
% $\nu_1(\parallel \ALL\NEXT X \parallel) = \parallel \ALL\NEXT(\nu_0(\parallel \ALL\NEXT X \parallel)) \parallel = \parallel \ALL\NEXT(S) \parallel = S$.

% (b) $\nu_0(\parallel \top \wedge \ALL \NEXT X \parallel) = S$, then\\
% $\nu_1(\parallel \top \wedge \ALL \NEXT X \parallel) = \parallel \top \parallel \cap \parallel \ALL\NEXT(\nu_0(\parallel \ALL\NEXT X \parallel)) \parallel  = S$.

% It is evident that $\Mod(\nu X. \ALL\NEXT X) = \Mod(\nu X.(\top \wedge \ALL \NEXT X)) = \Mod(\ALL \GLOBAL \top)= \{(\Hm,s_0)| (\Hm,s_0) $ is an initial structure$\}$.

% (iii) (a) $\mu_0(\parallel \varphi \wedge \ALL\NEXT X \parallel) = \emptyset$, then \\
% $\mu_1(\parallel \varphi \wedge \ALL\NEXT X \parallel) =\parallel \varphi\parallel \cap \parallel \ALL\NEXT(\mu_0(\parallel \ALL\NEXT X \parallel)) \parallel = \parallel \varphi\parallel \cap \parallel \ALL\NEXT(\emptyset) \parallel =  \emptyset$.

% (b) $\mu_0(\parallel \bot \vee (\varphi \wedge \ALL \NEXT X) \parallel) = \emptyset$, then\\
% $\mu_1(\parallel \bot \vee (\varphi \wedge \ALL \NEXT X) \parallel) = \parallel \bot \parallel \cup (\parallel \varphi\parallel \cap  \parallel \ALL\NEXT(\mu_0(\parallel \ALL\NEXT X \parallel)) \parallel) = \emptyset$.

% It is evident that $\Mod(\mu X. \varphi \wedge \ALL\NEXT X) = \Mod( \mu X.(\bot \vee (\varphi \wedge \ALL \NEXT X))) = \Mod(\ALL \FUTURE \bot)= \emptyset$.

% (iv) We will prove this results by proving $\parallel \nu X. \varphi \vee \ALL\NEXT X \parallel_V^{\Hm} = \parallel \nu X.(\top \wedge (\varphi \vee \ALL \NEXT X)) \parallel_V^{\Hm}$ for all $\Hm$ and $V$. Therefore we also omit the superscript $\Hm$ and subscript $V$.

% (a) $\nu_0(\parallel \varphi \vee  \ALL\NEXT X \parallel) = S$, then\\
% $\nu_1(\parallel \varphi \vee \ALL\NEXT X \parallel) =\parallel \varphi\parallel \cup \parallel \ALL\NEXT(\nu_0(\parallel \ALL\NEXT X \parallel)) \parallel = \parallel \ALL\NEXT(S) \parallel = S$.

% (b) $\nu_0(\parallel \top \wedge (\varphi \vee \ALL \NEXT X) \parallel) = S$, then\\
% $\nu_1(\parallel \top \wedge (\varphi \vee \ALL \NEXT X) \parallel) = \parallel \top \parallel \cap (\parallel \varphi \parallel \cup \parallel \ALL\NEXT(\nu_0(\parallel \ALL\NEXT X \parallel)))\parallel)  = S$.

% It is evident that $\Mod(\nu X. \varphi \vee \ALL\NEXT X) = \Mod(\nu X.(\top \wedge (\varphi \vee \ALL \NEXT X))) = \Mod(\top)= \{(\Hm,s_0)| (\Hm,s_0) $ is an initial structure$\}$.

% \end{proof}



% \begin{proposition}\label{pro:conCTL2mu}
% Let $\varphi \in L\mu_1$ be a formula with for each variables $X$ in $\varphi$ there is $\varphi$ does not contain $X\wedge \psi$ or $X\vee \psi$ as a subformula for any formula $\psi$, i.e. each variables $X$ in $\varphi$ is in a form $T X$ with $T\in \{\ALL, \EXIST\}$. Then we have that there exists a \CTL\ formula $\varphi'$ such that $\varphi \equiv \varphi'$.
% \end{proposition}
% \begin{proof}
% We can We prove it by inducing on the structures of the formula.

% $\varphi=p$, $\varphi'$ is $\varphi$ itself.

% $\varphi=\varphi_1 \wedge \varphi_2$, it is apparent that $\varphi'\equiv \varphi_1'\wedge \varphi_2'$ by inductive assumption. The $\varphi=\varphi_1 \vee \varphi_2$ can be proved similarly.

% $\varphi= \neg \psi$, then $\varphi'\equiv \neg \psi'$ by inductive assumption.

% $\varphi = \mu X. \psi \vee T X$, then $\varphi'=T \FUTURE \psi'$ by the fixpoint characterization of \CTL.

% $\varphi = \mu X. \psi \wedge T X$, then $\varphi \equiv \mu X. \bot \vee (\psi \wedge T X) \equiv T(\psi' \UNTIL \bot)$ by Lemma~\ref{lem:addFT} and the fixpoint characterization of \CTL.

% $\varphi = \nu X. \psi \wedge T X$, then $\varphi'=T \GLOBAL \psi'$ by the fixpoint characterization of \CTL.

% $\varphi = \nu X. \psi \vee T X$, then $\varphi \equiv \mu X. \top \wedge (\psi \vee T X) \equiv T(\psi' \UNLESS \top)$ by Lemma~\ref{lem:addFT} and the fixpoint characterization of \CTL.

% $\varphi=\mu X \cdot \psi \vee(\varphi_1 \wedge T\NEXT X)$, then $\varphi'= T(\varphi_1' \UNTIL \psi')$.

% $\varphi=\nu X \cdot \psi \wedge (\varphi_1 \vee T\NEXT X)$, then $\varphi'= T(\varphi_1' \UNLESS \psi')$.
% \end{proof}

% Proposition~\ref{pro:conCTL2mu} implies that for each $\mu$-sentence $\varphi$ that is in the form of that in Proposition~\ref{pro:conCTL2mu} and set $V$ of atoms there exists a \CTL\ formula $\psi$ such that $\Muforget(\varphi, V) \equiv \CTLforget(\psi, V)$.
...
\textbf{Proposition}~\ref{pro:mu:forget:2} \textbf{(Homogeneity)}
 Let $V\subseteq\cal A$ and $\phi$ be a $\mu$-sentence, then we have: % and $Q\in \{\EXIST, \ALL\}$.
   \begin{enumerate}[(i)]
     \item $\Muforget(\ALL\NEXT\phi,V)\equiv \ALL\NEXT \Muforget(\phi,V)$.
     \item $\Muforget(\EXIST\NEXT\phi,V)\equiv\EXIST\NEXT \Muforget(\phi,V)$.
